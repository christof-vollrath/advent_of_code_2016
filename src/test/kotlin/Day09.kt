...
but because it's within a data section of another marker, it is not treated any differently from the A that comes after it. It has a decompressed length of 6.
...
fun handleMarker(iterator: CharIterator): String = buildSequence {
...
fun readFromIterator(iterator: CharIterator, nr: Int): String = buildSequence {
        repeat(nr) {
...
fun parseMarker(iterator: CharIterator): Pair<Int, Int> {
...
fun readMarker(iterator: CharIterator): String = buildSequence {
    while(iterator.hasNext()) {
        val c = iterator.next()
        if (c != ')') yield(c)
        else break
